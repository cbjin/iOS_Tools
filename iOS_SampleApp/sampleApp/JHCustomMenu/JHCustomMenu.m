...
- (instancetype)initWithDataArr:(NSArray *)dataArr origin:(CGPoint)origin width:(CGFloat)width rowHeight:(CGFloat)rowHeight
{
    ...
    if (self) {
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(KScreenW - width - LeftToView, origin.y + TopToView, width, rowHeight * dataArr.count) style:UITableViewStylePlain];
        _viewframe = self.tableView.frame;
...
- (void)dismissWithCompletion:(void (^)(JHCustomMenu *object))completion
{
    ...
    [UIView animateWithDuration:0.4 animations:^{
        weakSelf.alpha = 0;
//        weakSelf.tableView.frame = CGRectMake(KScreenW - LeftToView, weakSelf.origin.y + TopToView, 0, 0);
        _tableView.alpha = 0;
...
//    [colors[serie] setFill];
    ...
    CGContextMoveToPoint(context,
                          CGRectGetMaxX(_viewframe) - LeftToView * 2, TopToView - 7);//设置起点

    CGContextAddLineToPoint(context,CGRectGetMaxX(_viewframe) -LeftToView * 1.5, TopToView);
    
    
    CGContextAddLineToPoint(context,CGRectGetMaxX(_viewframe) - LeftToView * 2.5, TopToView);
